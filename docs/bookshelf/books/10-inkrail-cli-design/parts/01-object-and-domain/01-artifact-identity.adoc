...
它创建和识别标准多书书架，在项目配置中记录书架与书本，按标准书架规则推导每本书的 book entry、documentRoot、RDF12 TTL、RDF12 JSON-LD 和 watch globs，在项目查询前强制刷新过期投影，并使用 Oxigraph 对单本书 RDF12 TTL 执行 SPARQL。
...
`inkrail` 的完成条件不由命令存在决定。只有当项目记忆、标准书架 profile、source set 新鲜度、投影刷新、查询执行、输出表面、错误语义和验证标准共同成立时，`inkrail` 才作为该人工制品成立。
